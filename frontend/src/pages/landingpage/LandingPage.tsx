...
import { Workout } from '../workouts/workouts/Workouts.tsx';

const LandingPage = () => {
    const [workouts, setWorkouts] = useState<Workout[]>([]);

    //@ts-expect-error - Ignore this error as we dont have time to fix it.
    ...
    return (
        <div className='landingContainer'>
            <div className='landingWelcomeCont'>
                {user && user.image ? (
                    <img src={user.image} alt='Avatar' className='landingWelcomeProfilePicture' />
                ) : (
                    <img src='https://www.w3schools.com/howto/img_avatar.png' alt='Avatar' className='landingWelcomeProfilePicture' />
                )}
                <h1 style={{ fontSize: '2em', margin: '0', fontFamily: 'koulen', fontWeight: '200' }}>
                    LET'S GO, {user && user.name.split(' ')[0]}!
                </h1>
            </div>
                ...
                <div className='landingYourWorkoutInnerCont'>
                    {workouts && workouts.length > 0 ? (
                        workouts.map((workout) => (
                            <div
                                key={workout._id}
                                className='landingWorkoutCard'
                                onClick={() => {
                                    navigateTo(`/workout-details`, workout._id);
                                }}>
                                <img
                                    src={workout && workout.image ? workout.image : WorkoutPlaceholderImg}
                                    alt={workout.workoutName}
                                    className='landingWorkoutCardImg'
                                />
                    ...
                    {exercises.length > 0 ? (
                        exercises.map((excercise) => (
                            <div
                                ...
                                {excercise.Exercise_Image.length > 0 ? (
                                    <img src={excercise.Exercise_Image} alt='exercise' className='landingExerciseCardImg' />
                                ) : (
                                    <img src={WorkoutPlaceholderImg} alt='exercise' className='landingExerciseCardImg' />
                                )}
                                   <span style={{height: "50px"}}>
                                    <h5
                                        style={{
